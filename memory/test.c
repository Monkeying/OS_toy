...
main()
{
	...
	//=======================================================测试段内变量的地址分配，读写
	{
		...
		if (segmentAddr > 0){//段地址分配成功
			...
			_write("testProcess1",_char,sizeof(char),1,"a");
			printf("after write a, read result: %c\n", *((char *)_read("testProcess1",_char,sizeof(char),1)));
			//字符串
			ADDR _chars = _malloc("testProcess1",sizeof(char)*10);
			_write("testProcess1",_char,sizeof(char),11,"helloWorld\0");
			printf("after write helloWorld, read result: %s\n", (char *)_read("testProcess1",_char,sizeof(char),11));
			//整数
			int input = -2017;
			ADDR _int = _malloc("testProcess1",sizeof(int)*1);
			_write("testProcess1",_int,sizeof(int),1,&input);
			printf("after write %d, read result: %d\n", input, *((int *)_read("testProcess1",_int,sizeof(int),1)));
			//数组
			int array[5] = {1,2,3,4,5};
			ADDR _array = _malloc("testProcess1",sizeof(int)*5);
			_write("testProcess1",_array,sizeof(int),5,array);
			printf("after write %d, read result: %d\n",array[4],((int *)_read("testProcess1",_int,sizeof(int),5))[4]);
		}
		else{//段地址分配失败
			printf("testProcess1 CreateProcess failed return %d\n",segmentAddr);
		}
